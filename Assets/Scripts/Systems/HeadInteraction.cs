...
namespace Scripts.Systems
{
    ...
    public class HeadInteraction : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                new Condition("z < 20, wait > 0.001, z > 20, wait > 1, z > 20",
                    () => onHeadInteraction?.Invoke(HeadInteractionType.Left)),
                new Condition("z > -20, wait > 0.001, z < -20, wait > 1, z < -20",
                    () => onHeadInteraction?.Invoke(HeadInteractionType.Right)),
                new Condition("x > -50, wait > 0.001, x < -50, wait > 1, x < -50",
                    () => onHeadInteraction?.Invoke(HeadInteractionType.LookingUp)),
                new Condition("x < 50, wait > 0.001, x > 50, wait > 1, x > 50",
                    () => onHeadInteraction?.Invoke(HeadInteractionType.LookingDown)),
                new Condition(
                    "z > 10, wait < 1, z < -10, wait < 1, z > 10, wait < 1, z < -10, wait < 1, z > 10, wait < 1, z < -10",
                    () => onHeadInteraction?.Invoke(HeadInteractionType.Shaking)),
                new Condition(

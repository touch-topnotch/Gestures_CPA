...
namespace Authentication
{
    ...
    public class ChainAuthManager
    {
        ...
        } //registration code used during authentication
        ...
        {
            if (GetBearerToken() != null && RegisterCode == null)
            {
                //we are already logged in
                throw new Exception(
                    "Already logged in succesfully. If desired, logout & re-register before calling Magic Link");
            }
        ...
        public async Task<List<Dictionary<string, string>>> GetERC20Tokens(CHAIN chain)
        {
            ...
            try
            {
                List<Dictionary<string, string>> jsonMap =

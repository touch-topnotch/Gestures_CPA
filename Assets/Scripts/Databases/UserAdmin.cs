...
namespace Scripts.Databases
{
    public static class UserAdmin
    {
        ...
        public static void Add(UserData userData)
        {
            ...
            List<UserData> userLibrary =
                JsonConvert.DeserializeObject<List<UserData>>(DataChanel.Get(databasePath, (ulong)userData.id));
        ...
        {
            List<UserData> userLibrary =
                JsonConvert.DeserializeObject<List<UserData>>(DataChanel.Get(databasePath, (ulong)id));
            for (int i = 0; i < userLibrary.Count; i++)
            {
